...
// =========================================================
// AGENT WITHDRAW (RETRAIT ESPÈCES) v7.1 — Direct Transf'air
// ✅ v7.0 : navigation vers reçu imprimable
// ✅ v7.1 :
...
//   - cardBorder : gris neutre (était violet pâle)
// =========================================================
...
const C = {
  violet:       "#6C47FF",   // violet conservé pour éléments de formulaire
  violetLight:  "#F5F3FF",
  violetBorder: "#EDE9FE",
  ...
  pageBg:       "#FFFFFF",
  white:        "#FFFFFF",
  cardBorder:   "#E8EDF5",   // ✅ gris neutre (était #EDE9FE violet)
  inputBg:      "#F4F7FA",
  ink:          "#12082E",
  inkMid:       "#4B3F72",
  inkSoft:      "#8B80A8",
...
// ─── Info Row ─────────────────────────────────────────────
function InfoRow({ label, value, icon, color = C.violet }: {
...
export default function AgentWithdrawScreen() {
  ...
  const getAmountData = () => {
    ...
    if (toNum(transaction.receivedAmount) > 0)
      return { val: fmt(toNum(transaction.receivedAmount), transaction.targetCurrency ?? "GNF"), curr: transaction.targetCurrency ?? "GNF" };
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      const receiptData = {
        ...
        reference:        transaction?.reference ?? code.trim(),
        date:             new Date().toISOString(),
        amount:           toNum(transaction?.amount ?? 0),
        currency:         transaction?.currency ?? "XOF",
        receivedAmount:   toNum(transaction?.receivedAmount) > 0 ? toNum(transaction?.receivedAmount) : undefined,
        targetCurrency:   transaction?.targetCurrency || undefined,
        exchangeRate:     transaction?.exchangeRate ? toNum(transaction.exchangeRate) : undefined,
        beneficiaryName:  transaction?.beneficiary?.fullName ?? "—",
        beneficiaryPhone: transaction?.beneficiary?.phone   || undefined,
        senderName,
        senderCountry:    transaction?.originCountry || transaction?.sender?.country || undefined,
        code:             code.trim(),
    ...
    <SafeAreaView style={s.safe}>
      ...
      <LinearGradient colors={[AGENT_BLUE, AGENT_BLUE_DARK]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.hero}>
        ...
        <View style={s.heroTopRow}>
          ...
          <View style={{ flex: 1 }} />
          {transaction ? (
            <TouchableOpacity style={s.iconBtn} onPress={resetForm}>
            ...
            <View style={s.centerBox}>
              ...
              <View style={[s.codeWrap, code.trim().length >= 6 && { borderColor: C.violet }]}>
                <TextInput
                  ...
                  placeholder="• • • • • •" placeholderTextColor={C.inkSoft}
                  keyboardType="default" autoCapitalize="characters" maxLength={20} textAlign="center"
                />
              </View>
              <TouchableOpacity
...
const s = StyleSheet.create({
  ...
  glow:       { position: "absolute", top: -50, right: -30, width: 150, height: 150, borderRadius: 75, backgroundColor: C.heroGlow },
  ...
  qrBox:     { width: 96, height: 96, borderRadius: 24, marginBottom: 20, backgroundColor: C.violetLight, borderWidth: 1.5, borderColor: C.violetBorder, justifyContent: "center", alignItems: "center" },
  qrHint:    { color: C.inkSoft, fontSize: 13, fontWeight: "600", textAlign: "center", marginBottom: 24, lineHeight: 19, paddingHorizontal: 10 },
  codeWrap:  { width: "100%", marginBottom: 22, backgroundColor: C.white, borderWidth: 2, borderColor: C.cardBorder, borderRadius: C.r.md, shadowColor: C.violet, shadowOpacity: 0.08, shadowRadius: 12, elevation: 4 },
  codeInput: { paddingVertical: 22, paddingHorizontal: 20, fontSize: 30, color: C.ink, fontWeight: "900", letterSpacing: 8 },
  cta:       { width: "100%", borderRadius: C.r.md, overflow: "hidden", shadowColor: C.violet, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.22, shadowRadius: 12, elevation: 5 },
  ...
  // Résultat
  validBadge: { flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: C.greenBg, borderRadius: C.r.md, padding: 13, borderWidth: 1, borderColor: C.greenBorder, marginBottom: 14 },
  ...
  amtCard: { backgroundColor: C.greenBg, borderRadius: C.r.xl, padding: 22, marginBottom: 14, borderWidth: 1.5, borderColor: C.greenBorder, shadowColor: C.green, shadowOpacity: 0.10, shadowRadius: 12, elevation: 4 },
  ...
  // ✅ v7.1 : cards avec ombre accentuée sur fond blanc
  card:   { backgroundColor: C.white, borderRadius: C.r.lg, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: C.cardBorder, shadowColor: "#64748B", shadowOpacity: 0.09, shadowRadius: 14, elevation: 5 },
  ...
  payBtn:      { borderRadius: C.r.md, overflow: "hidden", marginBottom: 10, shadowColor: C.green, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.22, shadowRadius: 12, elevation: 5 },
