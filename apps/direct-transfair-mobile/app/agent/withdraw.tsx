...
// =========================================================
// AGENT WITHDRAW (RETRAIT ESPÈCES) v7.0 — Direct Transf'air
// Design: Thème clair · Bleu #2563EB (héro) · Violet (form)
// ✅ v5.0 : Vérification code → Détail → Confirmation paiement
// ✅ v5.0 : Montant converti devise cible prioritaire
// ✅ v6.0 : Héro bleu unifié (arc concave, pill "• AGENT")
// ✅ v7.0 : Après paiement → navigation vers reçu imprimable
//           (router.push /agent/receipt avec data JSON encodé)
// =========================================================
...
// ─── Design System ──────────────────────────────────────
const C = {
  violet:       "#6C47FF",   // violet conservé pour les éléments de formulaire
  violetLight:  "#F5F3FF",
  violetBorder: "#EDE9FE",
  heroGlassBg:  "rgba(255,255,255,0.14)",
...
export default function AgentWithdrawScreen() {
  ...
  const { user }    = useAuth();   // ✅ v7 : pour alimenter agencyName / agentName dans le reçu
  ...
  // ✅ v7 : processPayment navigue vers le reçu au lieu d'un simple alert
  const processPayment = async () => {
    ...
    try {
      ...
      const receiptData = {
        ...
        receivedAmount:   toNum(transaction?.receivedAmount) > 0
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.hero}>
        ...
        {/* Ligne 1 : [← retour] [• AGENT] spacer [action] */}
        ...
        >
          ...
          {!transaction ? (
            ...
            <Animated.View style={{ opacity: fadeAnim, transform: [{ scale: scaleAnim }] }}>
              ...
              {/* ✅ v7 : CTA → déclenche processPayment → reçu */}
              <TouchableOpacity
...
const s = StyleSheet.create({
  ...
  iconBtn:    { width: 30, height: 30, borderRadius: 9, backgroundColor: C.heroGlassBg, borderWidth: 1, borderColor: C.heroGlassBdr, justifyContent: "center", alignItems: "center" },
  ...
  codeWrap: { width: "100%", marginBottom: 22, backgroundColor: C.white, borderWidth: 2, borderColor: C.cardBorder, borderRadius: C.r.md, shadowColor: C.violet, shadowOpacity: 0.07, shadowRadius: 10, elevation: 3 },
  ...
  amtCard: { backgroundColor: C.greenBg, borderRadius: C.r.xl, padding: 22, marginBottom: 14, borderWidth: 1.5, borderColor: C.greenBorder },
  ...
  card:   { backgroundColor: C.white, borderRadius: C.r.lg, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: C.cardBorder, shadowColor: C.violet, shadowOpacity: 0.04, shadowRadius: 6, elevation: 1 },
