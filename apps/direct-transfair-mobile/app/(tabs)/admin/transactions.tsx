...
// =========================================================
// ADMIN TRANSACTIONS v6.0 — Direct Transf'air
// ✅ Thème CLAIR
// ✅ Cartes COMPACTES — zéro action inline
// ✅ Modal d'actions au clic sur la carte
// ✅ COMPANY_ADMIN : badge "Payé" · actions Supprimer + Modifier montant
// ✅ SUPER_ADMIN   : badge "Reçu" · actions Valider + Rejeter
// =========================================================
...
const STATUS_CONFIG: Record<string, {
  // label vu par COMPANY_ADMIN | label vu par SUPER_ADMIN
...
function StatusBadge({ status, isSA }: { status: string; isSA: boolean }) {
  ...
  const label = isSA ? cfg.labelSA : cfg.labelAdmin;
  return (
    <View style={[sbS.pill, { backgroundColor: cfg.bg, borderColor: `${cfg.color}35` }]}>
      <Ionicons name={cfg.icon as any} size={10} color={cfg.color} />
      <Text style={[sbS.txt, { color: cfg.color, fontFamily: T.font.sans }]}>{label}</Text>
    </View>
  );
}
...
// ─── Tx Card COMPACTE ─────────────────────────────────────
// Aucune action inline — tout passe par le modal au clic
...
// ─── Modal d'actions ──────────────────────────────────────
// COMPANY_ADMIN : Supprimer + Modifier montant
// SUPER_ADMIN   : Valider + Rejeter
function TxActionModal({ visible, onClose, tx, isSA, accent, accentLt, onRefresh }: {
  ...
  // ── SUPER_ADMIN : Valider ou Rejeter ──
  ...
  // ── COMPANY_ADMIN : Modifier montant ──
      ...
      <View style={maS.overlay}>
        <View style={maS.sheet}>
          ...
          <View style={maS.header}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[maS.ref, { fontFamily: T.font.mono }]} numberOfLines={1}>
                {tx.reference}
              </Text>
              <Text style={[maS.date, { fontFamily: T.font.sans }]}>
          ...
          <ScrollView
            contentContainerStyle={maS.body}
          ...
          >
            ...
            <View style={maS.detailCard}>
              <View style={maS.detailRow}>
                ...
                <Text style={[maS.detailVal, { fontFamily: T.font.display, color: T.ink }]}>
                  {fmt(amount, tx.currency)} {tx.currency}
                </Text>
              </View>
              {fees > 0 && (
                <View style={maS.detailRow}>
                  <Text style={[maS.detailLabel, { fontFamily: T.font.sans }]}>FRAIS</Text>
                  <Text style={[maS.detailVal, { fontFamily: T.font.mono, color: T.inkSub }]}>
                    {fmt(fees, tx.currency)}
                  </Text>
                </View>
              )}
              {tx.sender && (
                <View style={maS.detailRow}>
                  ...
                  <Text style={[maS.detailVal, { fontFamily: T.font.sans, color: T.ink }]}>
            ...
            {isSA && (
              <>
                <View style={[maS.roleBanner, { backgroundColor: T.violetLt, borderColor: T.violetMd }]}>
                  ...
                  <Text style={[maS.roleBannerTxt, { color: T.violet, fontFamily: T.font.sans }]}>
                    Super Admin · Vous recevez ce montant
                  </Text>
                </View>
                ...
                {isPending ? (
                  <View style={maS.actionsCol}>
                    <TouchableOpacity
                      style={[maS.actionBtn, { backgroundColor: T.tealLt, borderColor: T.tealMd }]}
                    ...
                    >
                      {loading
                        ...
                        : <>
                            ...
                            <Text style={[maS.actionTxt, { color: T.teal, fontFamily: T.font.sans }]}>
                              Valider · Créditer mon compte
                            </Text>
                          </>
                      }
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity
                      style={[maS.actionBtn, { backgroundColor: T.redLt, borderColor: `${T.red}35` }]}
                      onPress={handleReject}
                      disabled={loading}
                    >
                      <Ionicons name="close-circle-outline" size={20} color={T.red} />
                      <Text style={[maS.actionTxt, { color: T.red, fontFamily: T.font.sans }]}>
                        Rejeter · Rembourser l'admin
                      </Text>
                    </TouchableOpacity>
                  </View>
                ) : (
                  <View style={maS.noActionBox}>
                    ...
                    <Text style={[maS.noActionTxt, { fontFamily: T.font.sans }]}>
                      Transaction déjà traitée — aucune action disponible
                    </Text>
                  </View>
                )}
              </>
            ...
            {!isSA && (
              <>
                <View style={[maS.roleBanner, { backgroundColor: T.skyLt, borderColor: T.skyMd }]}>
                  ...
                  <Text style={[maS.roleBannerTxt, { color: T.sky, fontFamily: T.font.sans }]}>
                    Admin Société · Gestion de vos transactions
                  </Text>
                </View>
                ...
                {!editMode ? (
                  <View style={maS.actionsCol}>
                    <TouchableOpacity
                      style={[maS.actionBtn, { backgroundColor: T.skyLt, borderColor: T.skyMd }]}
                      onPress={() => { setNewAmount(String(amount)); setEditMode(true); }}
                      disabled={loading}
                    >
                      <Ionicons name="pencil-outline" size={20} color={T.sky} />
                      <Text style={[maS.actionTxt, { color: T.sky, fontFamily: T.font.sans }]}>
                        Modifier le montant
                      </Text>
                    </TouchableOpacity>
                  ...
                  <View style={maS.editBlock}>
                    ...
                    <View style={maS.editRow}>
                      <TextInput
                        ...
                        placeholder={fmt(amount, tx.currency)}
                    ...
                    <View style={maS.editActions}>
                      <TouchableOpacity
                        style={[maS.editCancelBtn]}
                        onPress={() => { setEditMode(false); setNewAmount(""); }}
                        disabled={loading}
                      >
                        <Text style={[maS.editCancelTxt, { fontFamily: T.font.sans }]}>Annuler</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                        style={[maS.editConfirmBtn, { backgroundColor: T.sky }]}
...
const maS = StyleSheet.create({
  ...
  sheet: {
    ...
    maxHeight: "80%", borderWidth: 1, borderColor: T.border,
  },
  ...
  // Detail card
  detailCard: {
    backgroundColor: T.borderLt, borderRadius: T.radius.md,
    borderWidth: 1, borderColor: T.border, marginBottom: 16, overflow: "hidden",
  },
  detailRow: {
  ...
  roleBanner: {
    ...
    borderRadius: T.radius.md, borderWidth: 1, marginBottom: 14,
  },
  ...
  actionBtn: {
    ...
    padding: 16, borderRadius: T.radius.md, borderWidth: 1.5,
  },
  ...
  editCurrTag: {
    ...
    borderColor: T.skyMd, backgroundColor: T.skyLt,
    ...
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.header}>
        ...
        <View style={[s.roleBadge, { backgroundColor: roleTheme.lt, borderColor: roleTheme.md }]}>
          <Text style={[s.roleTxt, { color: accent, fontFamily: T.font.sans }]}>
            {roleTheme.label}
          </Text>
        </View>
        <TouchableOpacity
          style={[s.iconBtn, { backgroundColor: `${accent}12` }]}
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            ...
            >
              <Text style={[s.filterTxt, { fontFamily: T.font.sans, color: isActive ? (cfg?.color ?? accent) : T.inkSub }]}>
              ...
              <View style={[s.filterCount, { backgroundColor: isActive ? `${(cfg?.color ?? accent)}15` : T.pageBg }]}>
                <Text style={[s.filterCountTxt, { color: isActive ? (cfg?.color ?? accent) : T.inkMuted, fontFamily: T.font.mono }]}>
                  {count}
                </Text>
              </View>
            </TouchableOpacity>
