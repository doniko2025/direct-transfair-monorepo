...
export default function AdminTransactionsScreen() {
  ...
  const handleUpdateStatus = async (
    ...
    try {
      ...
      // ✅ Cas B2B : endpoints dédiés (ton backend fait débit/remboursement)
      if (isB2B) {
        if (newStatus === "VALIDATED") {
          await api.validateBankTransfer(tx.id);
        } else if (newStatus === "CANCELLED") {
          await api.rejectBankTransfer(tx.id);
        } else {
    ...
    return (
      <View style={styles.card}>
        ...
        <Text style={styles.fees}>
          Frais: {Number(item.fees).toLocaleString("fr-FR")} {item.currency}
        </Text>
        ...
        {item.status === "PENDING" && (
          <View style={styles.actions}>
            ...
            <TouchableOpacity
              ...
              onPress={() => handleUpdateStatus(item, "VALIDATED")}
            >
