...
// =========================================================
// PARAMÈTRES SOCIÉTÉ v1.0 — Direct Transf'air
// ✅ Réservé COMPANY_ADMIN uniquement
// ✅ Taux de change : toutes combinaisons GNF/EUR/XOF/USD/GBP
// ✅ Commissions : répartition Filiale vs Partenaire
// ✅ Thème bleu ciel cohérent avec le dashboard admin
// =========================================================
...
// Génère toutes les paires directionnelles possibles (A→B ≠ B→A)
function getAllPairs(): Array<{ from: CurrencyCode; to: CurrencyCode; key: string }> {
  ...
  for (const from of CURRENCIES) {
    for (const to of CURRENCIES) {
      if (from !== to) {
        pairs.push({ from, to, key: `${from}/${to}` });
      }
    }
  }
  return pairs;
...
const ALL_PAIRS = getAllPairs(); // 20 paires (5×4)

// Taux par défaut indicatifs
...
// ─── Section Header ───────────────────────────────────────
function SectionHeader({ icon, title, color, desc }: {
  ...
  return (
    <View style={rrS.row}>
      ...
      <View style={rrS.pairBlock}>
        ...
        <View>
          <Text style={[rrS.pairTxt, { fontFamily: T.font.mono }]}>
            {from} → {to}
          </Text>
          <Text style={[rrS.pairSub, { fontFamily: T.font.sans }]} numberOfLines={1}>
            1 {fromMeta.symbol} en {toMeta.symbol}
          </Text>
        </View>
      </View>
      ...
      <View style={[
        rrS.inputBox,
        focused && { borderColor: toMeta.color, backgroundColor: toMeta.bg + "80" },
      ]}>
        <TextInput
        ...
        />
        <Text style={[rrS.suffix, { color: toMeta.color, fontFamily: T.font.sans }]}>
          {toMeta.symbol}
        </Text>
      </View>
    </View>
...
const rrS = StyleSheet.create({
  ...
  inputBox: {
    ...
    backgroundColor: T.borderLt, borderWidth: 1.5, borderColor: T.border,
    borderRadius: T.radius.md, paddingHorizontal: 10, paddingVertical: 8,
    minWidth: 120,
  },
...
// ─── Commission Slider ────────────────────────────────────
// Affiche une répartition plateforme/agence en pourcentage
function CommissionSlider({
  ...
  return (
    <View style={csS.wrap}>
      ...
      {/* Boutons de réglage par paliers */}
...
export default function SettingsScreen() {
  ...
  // ── État commissions ──
  // Filiale : part plateforme (0-100)
  const [subsidPlatform,   setSubsidPlatform]   = useState(30);
  // Partenaire : part plateforme (0-100)
  const [partnerPlatform,  setPartnerPlatform]  = useState(50);
    ...
    try {
      ...
      if (Array.isArray(apiRates)) {
        for (const r of apiRates) {
          if (r.pair && r.rate !== undefined) {
            rateMap[r.pair] = String(r.rate);
          }
        }
      }
      // Remplir les manquants avec les defaults
      ...
      // Commissions
      try {
        const commRules = await api.getCommissionRules();
        if (Array.isArray(commRules) && commRules.length > 0) {
          const subsidRule  = (commRules as any[]).find((r: any) => r.sourceType === "SUBSIDIARY");
          const partnerRule = (commRules as any[]).find((r: any) => r.sourceType === "PARTNER");
          if (subsidRule?.platformShare  !== undefined) setSubsidPlatform(Math.round(subsidRule.platformShare));
          if (partnerRule?.platformShare !== undefined) setPartnerPlatform(Math.round(partnerRule.platformShare));
        }
      ...
      // Charger quand même les defaults
      const rateMap: Record<string, string> = {};
      for (const pair of ALL_PAIRS) {
    ...
    try {
      for (const pair of ALL_PAIRS) {
        ...
        if (!isFinite(val) || val <= 0) {
          errors.push(pair.key);
          continue;
        }
        try {
          await api.updateExchangeRate(pair.key, val);
        } catch { errors.push(pair.key); }
      }
      if (errors.length === 0) {
        Alert.alert("✅ Taux sauvegardés", "Tous les taux ont été mis à jour.");
      } else {
        Alert.alert(
          "⚠️ Partiellement sauvegardé",
          `${errors.length} paire(s) en erreur : ${errors.slice(0, 3).join(", ")}${errors.length > 3 ? "…" : ""}`,
        );
      }
  ...
  const handleSaveComm = async () => {
    ...
    try {
      await api.saveCommissionRule({
        type: "PERCENTAGE",
        value: subsidPlatform,
        sourceType: "SUBSIDIARY",
        destType: "SUBSIDIARY",
        platformShare: subsidPlatform,
        senderShare: 0,
        payerShare: 100 - subsidPlatform,
      } as any);
      await api.saveCommissionRule({
        type: "PERCENTAGE",
        ...
        senderShare: 0,
        payerShare: 100 - partnerPlatform,
      } as any);
      Alert.alert("✅ Commissions sauvegardées", "La répartition a été mise à jour.");
    } catch (e: any) {
      Alert.alert("Erreur", e?.response?.data?.message ?? "Sauvegarde impossible.");
    } finally { setSavingComm(false); }
  };

  // ── Groupement des paires par devise source ──
    ...
    <SafeAreaView style={s.safe}>
      ...
      >
        ...
        <View style={s.card}>
          ...
          {!ratesLoaded ? (
            ...
            CURRENCIES.map((from) => (
              <View key={from} style={s.rateGroup}>
                ...
                {/* Paires vers les autres devises */}
          ...
          {/* Bouton sauvegarder taux */}
          <TouchableOpacity
            style={[s.saveBtn, { backgroundColor: T.sky }, savingRates && { opacity: 0.65 }]}
          ...
          >
            {savingRates
              ...
              : <>
                  ...
                  <Text style={[s.saveTxt, { fontFamily: T.font.sans }]}>SAUVEGARDER LES TAUX</Text>
                </>
            }
          </TouchableOpacity>

          <View style={[s.infoBox, { backgroundColor: T.skyLt, borderColor: T.skyMd }]}>
            <Ionicons name="information-circle-outline" size={13} color={T.sky} />
            <Text style={[s.infoTxt, { fontFamily: T.font.sans }]}>
              Ces taux s'appliquent immédiatement aux nouvelles transactions. Les transactions en cours ne sont pas affectées.
            </Text>
          </View>
        </View>

        {/* ══════════════════════════════════════════════
            SECTION 2 — RÉPARTITION COMMISSIONS
        ══════════════════════════════════════════════ */}
        <View style={s.card}>
          <SectionHeader
          ...
          {/* Filiale */}
          <CommissionSlider
            label="Agences Filiales"
            icon="business-outline"
            color={T.teal}
            bg={T.tealLt}
            platform={subsidPlatform}
            onPlatformChange={setSubsidPlatform}
          />
          ...
          {/* Récap comparatif */}
          <View style={s.commCompare}>
            <View style={[s.commBox, { borderColor: T.tealMd, backgroundColor: T.tealLt }]}>
              <Text style={[s.commBoxLabel, { fontFamily: T.font.sans, color: T.teal }]}>FILIALE</Text>
              <Text style={[s.commBoxVal, { fontFamily: T.font.mono, color: T.teal }]}>{subsidPlatform}% plateforme</Text>
              <Text style={[s.commBoxSub, { fontFamily: T.font.sans }]}>{100 - subsidPlatform}% agence</Text>
            </View>
            <View style={s.commArrow}>
              <Ionicons name="swap-horizontal-outline" size={16} color={T.inkMuted} />
            </View>
            <View style={[s.commBox, { borderColor: T.amberMd, backgroundColor: T.amberLt }]}>
              <Text style={[s.commBoxLabel, { fontFamily: T.font.sans, color: T.amber }]}>PARTENAIRE</Text>
              <Text style={[s.commBoxVal, { fontFamily: T.font.mono, color: T.amber }]}>{partnerPlatform}% plateforme</Text>
              <Text style={[s.commBoxSub, { fontFamily: T.font.sans }]}>{100 - partnerPlatform}% agence</Text>
            </View>
          </View>

          {/* Bouton sauvegarder commissions */}
          <TouchableOpacity
            style={[s.saveBtn, { backgroundColor: T.teal }, savingComm && { opacity: 0.65 }]}
          ...
          >
            {savingComm
              ...
              : <>
                  <Ionicons name="cloud-upload-outline" size={18} color={T.white} />
                  <Text style={[s.saveTxt, { fontFamily: T.font.sans }]}>SAUVEGARDER LES COMMISSIONS</Text>
                </>
            }
          </TouchableOpacity>

          <View style={[s.infoBox, { backgroundColor: T.tealLt, borderColor: T.tealMd }]}>
            <Ionicons name="information-circle-outline" size={13} color={T.teal} />
            <Text style={[s.infoTxt, { color: T.teal, fontFamily: T.font.sans }]}>
              Les filiales (propriété directe) bénéficient généralement d'un taux de commission plus avantageux que les partenaires externes.
            </Text>
          </View>
        </View>
...
const s = StyleSheet.create({
  ...
  card: {
    ...
    borderWidth: 1, borderColor: T.border, ...T.shadow.card,
  },

  rateGroup: { marginBottom: 14 },
  rateGroupHeader: {
    flexDirection: "row", alignItems: "center", gap: 8,
    paddingHorizontal: 10, paddingVertical: 7,
    borderRadius: T.radius.md, marginBottom: 4,
  },
  rateGroupTitle: { fontSize: 11, fontWeight: "900", letterSpacing: 0.5 },

  commDivider: { height: 1, backgroundColor: T.border, marginVertical: 16 },
  ...
  commBox: {
    flex: 1, borderRadius: T.radius.md, borderWidth: 1.5,
    padding: 12, alignItems: "center",
  },
  commBoxLabel: { fontSize: 8, fontWeight: "900", letterSpacing: 1, marginBottom: 4 },
  commBoxVal:   { fontSize: 16, fontWeight: "900" },
  commBoxSub:   { fontSize: 10, color: T.inkSub, fontWeight: "600", marginTop: 2 },
  commArrow:    { width: 32, alignItems: "center" },

  saveBtn: {
    flexDirection: "row", alignItems: "center", justifyContent: "center",
    gap: 8, paddingVertical: 15, borderRadius: T.radius.md, marginTop: 16,
  },
  ...
  infoTxt: { flex: 1, fontSize: 11, color: T.sky, fontWeight: "600", lineHeight: 17 },
});