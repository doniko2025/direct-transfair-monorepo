...
// =========================================================
// TRANSACTION DETAIL v5.1
// ✅ SuperAdmin : SERVICE_PAYMENT → badge "REÇUE" (vert)
//                 "Montant crédité" au lieu de "Total payé"
// ✅ CompanyAdmin/User : comportement inchangé
// =========================================================
...
const THEMES = {
  SUPER_ADMIN:   { primary: "#1956F0", light: "#EEF2FF", bg: "#F8FAFC" },
...
export default function TransactionDetailScreen() {
  ...
  const performAction = async (newStatus: "VALIDATED" | "CANCELLED") => {
    ...
    try {
      ...
      else if (isB2B && newStatus === "CANCELLED" && isSuperAdmin) await api.rejectBankTransfer(transaction.id);
      else if (isCompanyAdmin || isSuperAdmin) await api.adminUpdateTransactionStatus(transaction.id, newStatus);
  ...
  // ─── Identification du type de transaction ───────────
  ...
  // ✅ isIncoming dépend du rôle
  const isIncoming = isSuperAdmin
    ? (isB2B || isDeposit || isRefill)          // SuperAdmin : B2B = encaissement
    : (isDeposit || transaction.beneficiaryId === user?.id || (isRefill && transaction.agencyId === user?.agencyId));
  ...
  const showSuperAdminActions = Boolean(isSuperAdmin && transaction.status === "PENDING");
  ...
  // ✅ Badge statut : "REÇUE" pour SuperAdmin sur B2B payé
  const getStatusInfo = (status: string) => {
    if (status === "PAID" && isIncoming && isSuperAdmin) {
      ...
      <ScrollView contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.ticket}>
          ...
          {showWithdrawalCode && (
            <View style={s.codeSection}>
              ...
              <TouchableOpacity style={s.codeBox} onPress={handleCopyCode} activeOpacity={0.7}>
                <Text style={[s.codeText, transaction.status === "CANCELLED" && s.codeStrikethrough]}>
                  {transaction.reference}
                </Text>
          ...
          <View style={s.detailsSection}>
            ...
            {isDeposit ? (
              <>
                ...
                {transaction.agency ? (
                  <>
                    ...
                    {transaction.agency.managerName && <DetailRow label="Gérant" value={transaction.agency.managerName} sub />}
                    <DetailRow label="Ville" value={transaction.agency.city || ""} sub />
                  </>
                ) : (
              ...
              <>
                ...
                <View style={s.divider} />
                {/* ✅ Pour SuperAdmin : bénéficiaire = "Plateforme (moi)" */}
                <DetailRow
                  label="Bénéficiaire"
                  value={isSuperAdmin && isB2B ? "Plateforme (moi)" : (isB2B ? "Super Admin" : transaction.beneficiary?.fullName || "Non spécifié")}
                />
          ...
          <View style={s.financeSection}>
            ...
            {/* ✅ Label et couleur selon direction */}
            <DetailRow
              label={isIncoming ? "Montant crédité" : "Total payé"}
              value={`${Number(transaction.total || transaction.amount).toLocaleString("fr-FR")} ${transaction.currency}`}
              bold
              color={isIncoming ? "#059669" : theme.primary}
            />
          </View>
        </View>
        ...
        {showSuperAdminActions && (
          <View style={s.adminActions}>
            ...
            <View style={{ flexDirection: "row", gap: 12 }}>
              <TouchableOpacity
                style={[s.actionBtn, { backgroundColor: "#FEE2E2", borderWidth: 1, borderColor: "#FECACA" }]}
        ...
        {canUserCancel && (
          ...
          >
            {processing
              ...
              : <Text style={s.cancelBtnText}>Annuler la transaction</Text>
            }
          </TouchableOpacity>
        )}
      </ScrollView>
...
function DetailRow({ label, value, bold, sub, color }: any) {
  return (
    <View style={[s.row, sub && { marginTop: 4 }]}>
      ...
      <Text style={[
        s.rowValue,
        bold && { fontWeight: "900", color: color || "#0F172A", fontSize: 15 },
