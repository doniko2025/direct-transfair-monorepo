...
// =========================================================
// SECURITY v5.1 — Direct Transf'air
// ✅ Fix web : outlineStyle inline + underlineColorAndroid prop
// ✅ Fix validation : trim() sur valeurs state, pas sur event
      ...
      <View style={[piS.box, focused && { borderColor: accent, borderWidth: 1.5 }]}>
        ...
        <TextInput
          style={[
            ...
            // ✅ outlineStyle INLINE — jamais dans StyleSheet
            Platform.OS === "web" && ({ outlineStyle: "none" } as any),
          ]}
          value={value}
          // ✅ onChange direct, pas de trim ici — on trim uniquement au moment de la validation
...
export default function SecurityScreen() {
  ...
  const [toast, setToast] = useState({
    title: "", message: "", type: "success" as "success" | "error",
  });
  ...
  const handleSave = async () => {
    // ✅ Trim uniquement ici, au moment de la validation
    const oldTrimmed     = oldPass.trim();
    const newTrimmed     = newPass.trim();
    const confirmTrimmed = confirmPass.trim();

    // ✅ Debug log pour identifier le problème sur web
    console.log("🔐 handleSave", {
      oldLen: oldTrimmed.length,
      newLen: newTrimmed.length,
      confirmLen: confirmTrimmed.length,
    });

    if (!oldTrimmed || !newTrimmed || !confirmTrimmed) {
      showToast("Champs manquants", `Remplissez tous les champs (${oldTrimmed.length}/${newTrimmed.length}/${confirmTrimmed.length}).`, "error");
      return;
    }
    ...
    try {
      ...
      const raw = e?.response?.data?.message || e?.message || "Impossible de modifier le code.";
  ...
  const strengthLevel =
    ...
    :                          "strong";
  const strengthColor =
    strengthLevel === "weak"   ? T.red
    : strengthLevel === "medium" ? T.amber
    : T.green;
  const strengthLabel =
    strengthLevel === "weak"   ? "Trop court"
    : strengthLevel === "medium" ? "Acceptable"
    : "Fort ✓";

  const canSave = oldPass.trim().length > 0 && newPass.trim().length >= 6 && confirmPass.trim().length > 0;
            ...
            <View style={s.card}>
              ...
              {newPass.length > 0 && (
                <View style={s.strengthRow}>
                  ...
                  <Text style={[s.strengthTxt, { color: strengthColor, fontFamily: T.font.sans }]}>
                    {strengthLabel}
                  </Text>
                </View>
              )}
              ...
              {confirmPass.length > 0 && newPass.length > 0 && (
                <View style={[s.matchRow, {
                  ...
                  <Text style={[
                    s.matchTxt,
                    { color: newPass === confirmPass ? T.green : T.red, fontFamily: T.font.sans },
                  ]}>
...
const s = StyleSheet.create({
  ...
  scroll: {
    paddingHorizontal: 20, paddingTop: 8, paddingBottom: 30, alignItems: "center",
  },
  ...
  subtitle: {
    ...
    textAlign: "center", lineHeight: 19, marginBottom: 28, paddingHorizontal: 20,
  },
